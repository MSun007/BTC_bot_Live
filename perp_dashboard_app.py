...
function renderLarryMindset(d){
 const es=d.engine_state||{}, diag=es.entry_diagnostics||es.last_entry_diagnostics||d.entry_diagnostics||{}, cfg=d.config||{};
 ...
 if(!riskOk){decision='BLOCKED — risk gate is closed';reason=(es.risk_gate||{}).reason||'New entries are disabled by the active risk gate.';badge='BLOCKED';badgeClass='bad';activeStep='mindsetRegimeStep';}
   ...
   activeStep='mindsetExitStep'; badge=pr.tsl_active?'PROTECTING':'IN POSITION'; badgeClass='good';
   if(pr.tsl_active){decision=`PROTECTING ${pr.side||direction} — trailing stop active`;reason=`Larry is managing ${num(contracts,0)} contracts with the trailing stop as the active exit.`;}
   else if(pc.tp1_done===true){decision=`HOLDING ${pr.side||direction} — TP step completed`;reason='The first profit-taking step is complete; Larry is waiting for trailing-stop activation or another exit condition.';}
   else {decision=`HOLDING ${pr.side||direction} — managing the open trade`;reason=pr.status||'ATR protection is active while Larry watches the profit-taking thresholds.';}
 ...
 const exitValue=!hasPosition?'Not armed':pr.tsl_active?'Trailing stop active':pc.tp1_done===true?'TP step done':'ATR stop active';
