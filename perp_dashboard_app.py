#!/usr/bin/env python3
"""
Coinbase Unified BTC Dashboard — v73 authenticated control plane
...
<title>Larry Dashboard — Sign in</title>
<style>
  body{background:#11151b;color:#e8eaee;font-family:-apple-system,"Segoe UI",system-ui,sans-serif;
       display:flex;align-items:center;justify-content:center;height:100vh;margin:0}
  form{background:#1a212c;border:1px solid #2b3444;border-radius:10px;padding:32px 28px;width:280px}
  h1{font-size:18px;margin:0 0 18px}
  input{width:100%;box-sizing:border-box;padding:10px 12px;font-size:16px;border-radius:6px;
        border:1px solid #2b3444;background:#11151b;color:#e8eaee;margin-bottom:14px}
  button{width:100%;padding:10px;border-radius:6px;border:none;background:#4fb3ab;color:#0b1210;
         font-weight:600;font-size:15px;cursor:pointer}
  .err{color:#ff9598;font-size:13px;margin-bottom:14px}
</style></head><body>
<form method="post" action="/login">
  <h1>Larry Dashboard</h1>
...
STRATEGY_CONFIG_ALLOWED_KEYS = set(DEFAULT_CONFIG.keys()) | {
    ...
    "MAX_CONVICTION_CONTRACTS", "CONTRACTS_PER_TRADE_FULL", "CONTRACTS_PER_TRADE_PARTIAL", "CONTRACTS_PER_TRADE_PROBE", "MAX_ABS_NET_CONTRACTS", "SCORE4_MACRO_OVERRIDE_ENABLED", "MACRO_BLOCKED_PROBE_CONTRACTS", "TP1_PCT", "TP1_FRACTION", "FUNDING_SIZE_REDUCE_AT", "ENABLE_SPOT_BTC_TRADING", "ENABLE_SPOT_BRIDGE_PERP_BUYS",
...
<style>
:root{--bg:#07111f;--card:#0d1b2d;--card2:#10243a;--line:#203a58;--text:#e8f1ff;--sub:#8fa8c4;--muted:#66819e;--green:#22c55e;--red:#ef4444;--orange:#f59e0b;--blue:#38bdf8;--purple:#a78bfa;--yellow:#facc15}
*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,#10233d 0,#07111f 35%,#050912 100%);color:var(--text);font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif}.app{max-width:1440px;margin:0 auto;padding:14px}.hero{background:linear-gradient(135deg,#0f2a48,#0b1727);border:1px solid var(--line);border-radius:22px;padding:18px;box-shadow:0 18px 50px rgba(0,0,0,.28);margin-bottom:14px}.hero-row{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.brand{font-weight:900;font-size:1.35rem;letter-spacing:-.02em}.sub{color:var(--sub);font-size:.82rem;margin-top:3px}.price{font-size:1.45rem;font-weight:900;text-align:right}.pill-row{display:flex;flex-wrap:wrap;gap:8px;margin-top:13px}.pill{padding:7px 10px;border-radius:999px;font-size:.74rem;font-weight:800;border:1px solid rgba(255,255,255,.08)}.good{color:var(--green)}.bad{color:var(--red)}.warn{color:var(--orange)}.blue{color:var(--blue)}.purple{color:var(--purple)}.muted{color:var(--muted)}.pill.good{background:rgba(34,197,94,.12)}.pill.bad{background:rgba(239,68,68,.12)}.pill.warn{background:rgba(245,158,11,.12)}.pill.blue{background:rgba(56,189,248,.12)}.pill.purple{background:rgba(167,139,250,.12)}.warnbox,.errbox{display:none;border-radius:14px;padding:11px 13px;margin:12px 0;font-size:.85rem}.warnbox{background:rgba(245,158,11,.10);border:1px solid rgba(245,158,11,.35);color:#ffd38a}.errbox{background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.35);white-space:pre-wrap}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:12px}.card{background:rgba(13,27,45,.96);border:1px solid var(--line);border-radius:18px;padding:15px;box-shadow:0 12px 32px rgba(0,0,0,.18)}.span3{grid-column:span 3}.span4{grid-column:span 4}.span5{grid-column:span 5}.span6{grid-column:span 6}.span7{grid-column:span 7}.span8{grid-column:span 8}.span12{grid-column:span 12}h2{margin:0 0 12px;font-size:.95rem;letter-spacing:.02em}h3{margin:0 0 8px;font-size:.82rem;color:var(--sub);font-weight:800}.big{font-size:1.65rem;font-weight:950;letter-spacing:-.03em}.kv{display:flex;justify-content:space-between;gap:10px;border-top:1px solid rgba(255,255,255,.06);padding:9px 0}.kv:first-of-type{border-top:0}.k{color:var(--sub);font-size:.78rem}.v{font-weight:800;font-size:.82rem;text-align:right}.mini{font-size:.72rem;color:var(--muted)}.metric-row{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.metric{background:var(--card2);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:11px}.metric .label{font-size:.72rem;color:var(--sub)}.metric .val{font-size:1.05rem;font-weight:900;margin-top:5px}.bar{height:10px;background:#07101c;border-radius:999px;overflow:hidden;border:1px solid rgba(255,255,255,.06)}.fill{height:100%;width:0%;background:var(--blue);border-radius:999px}.gauge{position:relative;height:14px;background:#07101c;border-radius:999px;border:1px solid rgba(255,255,255,.08);overflow:hidden}.zone{position:absolute;top:0;height:100%;background:rgba(148,163,184,.22)}.sig-card.active .zone{background:rgba(34,197,94,.55);box-shadow:0 0 18px rgba(34,197,94,.35)}.needle{position:absolute;top:-3px;width:3px;height:20px;background:#fff;border-radius:2px;box-shadow:0 0 8px rgba(255,255,255,.7)}.sig-card.active .needle{background:var(--green);box-shadow:0 0 12px rgba(34,197,94,.9)}.sig-card{background:var(--card2);border:1px solid rgba(255,255,255,.06);border-radius:15px;padding:12px;margin-bottom:10px}.sig-card.active{border-color:rgba(34,197,94,.65);background:linear-gradient(180deg,rgba(34,197,94,.13),rgba(15,23,42,.82))}.sig-head{display:flex;justify-content:space-between;margin-bottom:8px}.sig-name{font-weight:900}.sig-val{font-weight:900}.sig-desc{display:flex;justify-content:space-between;color:var(--sub);font-size:.72rem;margin-top:5px}.score-pips{display:flex;gap:6px}.score-pip{height:14px;flex:1;border-radius:999px;background:#17273b}.score-pip.on{background:linear-gradient(90deg,var(--green),#86efac)}.ladder{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.slot{background:#07101c;border:1px solid var(--line);border-radius:13px;padding:12px;text-align:center}.slot.on{background:rgba(34,197,94,.14);border-color:rgba(34,197,94,.55)}.slot.manual{background:rgba(167,139,250,.14);border-color:rgba(167,139,250,.5)}.slot .num{font-size:1.1rem;font-weight:950}.slot .lbl{font-size:.7rem;color:var(--sub);margin-top:3px}.table-wrap{overflow:auto}.table{width:100%;border-collapse:collapse;font-size:.78rem}.table th{text-align:left;color:var(--sub);font-weight:800;border-bottom:1px solid var(--line);padding:8px}.table td{border-bottom:1px solid rgba(255,255,255,.06);padding:8px;white-space:nowrap}.chartbox{height:230px}.hist-row{display:flex;align-items:center;gap:6px;border-bottom:1px solid rgba(255,255,255,.06);padding:7px 0}.hist-ts{width:62px;color:var(--sub);font-size:.72rem}.hist-score{width:24px;height:22px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-weight:900;background:#17273b}.hist-score.s3{background:rgba(245,158,11,.22);color:var(--orange)}.hist-score.s4{background:rgba(34,197,94,.22);color:var(--green)}.sig-pip{width:18px;height:18px;border-radius:5px;display:inline-flex;align-items:center;justify-content:center;font-size:.62rem;font-weight:900;margin-right:3px}.sp-on{background:rgba(34,197,94,.24);color:var(--green)}.sp-off{background:#17273b;color:var(--muted)}
.status-banner{display:flex;align-items:center;justify-content:space-between;gap:12px;border-radius:16px;padding:14px 16px;margin-bottom:12px;font-weight:950;border:1px solid rgba(255,255,255,.10)}.status-banner.enabled{background:rgba(34,197,94,.14);border-color:rgba(34,197,94,.48);color:#86efac}.status-banner.blocked{background:rgba(239,68,68,.14);border-color:rgba(239,68,68,.48);color:#fecaca}.status-banner.unknown{background:rgba(245,158,11,.12);border-color:rgba(245,158,11,.42);color:#fed7aa}.macro-card{background:linear-gradient(135deg,rgba(13,27,45,.98),rgba(12,37,64,.98));border-width:2px}.macro-top{display:grid;grid-template-columns:1.2fr 2fr;gap:14px;align-items:stretch}.macro-state{font-size:2.2rem;font-weight:1000;letter-spacing:-.04em;margin:8px 0}.macro-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:10px 0}.check.yes{color:var(--green)}.check.no{color:var(--red)}.pnl-note{font-size:.72rem;color:var(--sub);line-height:1.35;margin-top:8px}.method-badge{display:inline-block;padding:4px 8px;border-radius:999px;background:rgba(56,189,248,.12);color:var(--blue);font-size:.68rem;font-weight:900;margin-left:6px}.reference-row{background:rgba(148,163,184,.06);color:var(--sub)}.reference-row td:first-child{color:var(--yellow);font-weight:900}
.risk-hero{display:grid;grid-template-columns:1.1fr 2fr;gap:14px;align-items:stretch}.risk-main{background:linear-gradient(135deg,rgba(34,197,94,.12),rgba(15,23,42,.8));border:1px solid rgba(34,197,94,.28);border-radius:16px;padding:14px}.risk-main.short{background:linear-gradient(135deg,rgba(239,68,68,.13),rgba(15,23,42,.8));border-color:rgba(239,68,68,.35)}.risk-main.tsl-on{background:linear-gradient(135deg,rgba(34,197,94,.22),rgba(15,23,42,.85));border-color:rgba(34,197,94,.65);box-shadow:0 0 0 1px rgba(34,197,94,.10),0 12px 30px rgba(34,197,94,.08)}.risk-main.tsl-on{background:linear-gradient(135deg,rgba(34,197,94,.22),rgba(15,23,42,.85));border-color:rgba(34,197,94,.65);box-shadow:0 0 0 1px rgba(34,197,94,.10),0 12px 30px rgba(34,197,94,.08)}.risk-side{font-size:1.75rem;font-weight:1000;letter-spacing:-.03em}.risk-contracts{font-size:.9rem;color:var(--sub);margin-top:2px}.risk-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.risk-stop-card{margin-top:12px;background:#07101c;border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:12px}.stop-line{position:relative;height:16px;background:#111827;border-radius:999px;border:1px solid rgba(255,255,255,.08);overflow:hidden;margin:9px 0}.stop-progress{height:100%;background:linear-gradient(90deg,var(--blue),var(--green));width:0%;border-radius:999px}.stop-progress.active{background:linear-gradient(90deg,var(--green),#86efac);box-shadow:0 0 18px rgba(34,197,94,.35)}.risk-status{font-size:.78rem;color:var(--sub);line-height:1.35}.stop-badge{display:inline-block;border-radius:999px;padding:5px 8px;font-size:.68rem;font-weight:950;border:1px solid rgba(255,255,255,.10);background:rgba(148,163,184,.12)}.stop-badge.tsl{color:#86efac;background:rgba(34,197,94,.12);border-color:rgba(34,197,94,.35)}.stop-badge.atr{color:#fed7aa;background:rgba(245,158,11,.12);border-color:rgba(245,158,11,.35)}.stop-badge.manual{color:#bfdbfe;background:rgba(59,130,246,.14);border-color:rgba(59,130,246,.4)}
...
.monitor-pips{display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin:8px 0}.monitor-pip{padding:7px 8px;border-radius:10px;background:#17273b;color:var(--muted);font-size:.72rem;font-weight:800;border:1px solid rgba(255,255,255,.06)}.monitor-pip.on{background:rgba(34,197,94,.18);border-color:rgba(34,197,94,.55);color:#86efac}.monitor-pip.short.on{background:rgba(239,68,68,.18);border-color:rgba(239,68,68,.55);color:#fca5a5}.rule-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.rule-card{background:#07101c;border:1px solid var(--line);border-radius:12px;padding:10px}.rule-title{font-size:.72rem;color:var(--sub);font-weight:900}.rule-current{font-size:.82rem;font-weight:900;margin-top:4px}.rule-status{font-size:.68rem;margin-top:5px;font-weight:950}.rule-status.ok{color:var(--green)}.rule-status.active{color:var(--green)}.rule-status.open{color:var(--green)}.rule-status.monitored{color:var(--blue)}.rule-status.modified{color:var(--orange)}.rule-status.blocked,.rule-status.halted,.rule-status.paused{color:var(--red)}.rule-status.needs{color:var(--orange)}.risk-hero{display:grid;grid-template-columns:1.1fr 2fr;gap:14px;align-items:stretch}.risk-main{background:linear-gradient(135deg,rgba(34,197,94,.12),rgba(15,23,42,.8));border:1px solid rgba(34,197,94,.28);border-radius:16px;padding:14px}.risk-main.short{background:linear-gradient(135deg,rgba(239,68,68,.13),rgba(15,23,42,.8));border-color:rgba(239,68,68,.35)}.risk-side{font-size:1.75rem;font-weight:1000;letter-spacing:-.03em}.risk-contracts{font-size:.9rem;color:var(--sub);margin-top:2px}.risk-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.risk-stop-card{margin-top:12px;background:#07101c;border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:12px}.stop-line{position:relative;height:16px;background:#111827;border-radius:999px;border:1px solid rgba(255,255,255,.08);overflow:hidden;margin:9px 0}.stop-progress{height:100%;background:linear-gradient(90deg,var(--blue),var(--green));width:0%;border-radius:999px}.stop-progress.active{background:linear-gradient(90deg,var(--green),#86efac);box-shadow:0 0 18px rgba(34,197,94,.35)}.risk-status{font-size:.78rem;color:var(--sub);line-height:1.35}.stop-badge{display:inline-block;border-radius:999px;padding:5px 8px;font-size:.68rem;font-weight:950;border:1px solid rgba(255,255,255,.10);background:rgba(148,163,184,.12)}.stop-badge.tsl{color:#86efac;background:rgba(34,197,94,.12);border-color:rgba(34,197,94,.35)}.stop-badge.atr{color:#fed7aa;background:rgba(245,158,11,.12);border-color:rgba(245,158,11,.35)}.stop-badge.manual{color:#bfdbfe;background:rgba(59,130,246,.14);border-color:rgba(59,130,246,.4)}
@media(max-width:1000px){.rule-grid{grid-template-columns:repeat(2,1fr)}}
.pnl-visual{margin:12px 0;background:#07101c;border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:12px}.pnl-bar-title{font-size:.74rem;color:var(--sub);font-weight:900;margin-bottom:8px}.pnl-bar-row{display:grid;grid-template-columns:120px 1fr 90px;gap:8px;align-items:center;margin:7px 0}.pnl-bar-label{font-size:.72rem;color:var(--sub);font-weight:800}.pnl-bar-track{height:12px;background:#111827;border-radius:999px;overflow:hidden;border:1px solid rgba(255,255,255,.06)}.pnl-bar-fill{height:100%;width:0%;border-radius:999px;background:var(--green)}.pnl-bar-fill.neg{background:var(--red)}.pnl-bar-fill.neu{background:var(--blue)}.pnl-bar-value{font-size:.72rem;font-weight:900;text-align:right}
.diagnostic-card{background:rgba(9,20,35,.86);border-style:dashed}.diagnostic-card h2{color:#b6c6da}.headline-note{font-size:.72rem;color:var(--muted);margin-top:6px}.card h2 .method-badge{float:none}

.control-grid{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:12px;align-items:end}.control-grid label{font-size:12px;color:var(--muted);display:flex;flex-direction:column;gap:5px}.control-grid input,.control-grid select{background:#111827;border:1px solid #293548;color:var(--text);border-radius:10px;padding:10px;font-size:14px}.btn{background:#16c784;color:#04120b;border:0;border-radius:12px;padding:11px 14px;font-weight:800;cursor:pointer}.btn:hover{filter:brightness(1.1)}.btn-danger{background:#ef4444;color:#fff}.btn-warn{background:#f59e0b;color:#111827}.halt-panel{display:grid;grid-template-columns:1.2fr 1fr 1fr;gap:12px;align-items:center}.halt-status{border-radius:16px;padding:13px;border:1px solid rgba(255,255,255,.10);font-weight:950}.halt-status.on{background:rgba(239,68,68,.16);border-color:rgba(239,68,68,.55);color:#fecaca}.halt-status.off{background:rgba(34,197,94,.14);border-color:rgba(34,197,94,.48);color:#86efac}.halt-actions{display:flex;gap:10px;flex-wrap:wrap}.halt-actions .btn{min-width:110px}@media(max-width:760px){.halt-panel{grid-template-columns:1fr}.halt-actions .btn{width:100%}}@media(max-width:900px){.control-grid{grid-template-columns:1fr 1fr}.control-grid .btn{grid-column:1/-1;width:100%;padding:13px 14px}}@media(max-width:560px){.risk-grid{grid-template-columns:1fr}.control-grid{grid-template-columns:1fr}.control-grid label{font-size:11px}.control-grid input,.control-grid select{width:100%;box-sizing:border-box}.control-grid .btn{grid-column:1;width:100%}}
...
.trigger-lane-card{border:1px solid rgba(255,255,255,.08);border-radius:12px;background:#07101c;padding:9px}
...
function renderRiskIntelligence(d){
 ...
 if(m.active){ set('intelManualBook', `${m.side||'—'} ${num(m.contracts,0)} · ${usd(m.unrealized_pnl)}`); setClass('intelManualBook','intel-val '+cls(m.unrealized_pnl)); set('intelManualNote', `Entry ${usd(m.avg_entry_price)} · Mark ${usd(m.current_price)} · ${m.unrealized_pnl_pct==null?'—':pct(m.unrealized_pnl_pct)} · excluded from Larry P&L.`); }
 else { set('intelManualBook','No manual open book'); setClass('intelManualBook','intel-val muted'); set('intelManualNote','Manual positions will appear here and remain excluded from Larry strategy return.'); }
 ...
 set('intelSlippage', ex.sample_count?`${num(ex.avg_slippage_bps,2)} bps avg`:'No samples'); setClass('intelSlippage','intel-val '+(ex.avg_slippage_bps>0?'warn':'good')); set('intelSlippageNote', `Last ${ex.last_slippage_bps==null?'—':num(ex.last_slippage_bps,2)+' bps'} · Worst ${ex.worst_slippage_bps==null?'—':num(ex.worst_slippage_bps,2)+' bps'} · Samples ${ex.sample_count||0}.`);
 ...
 set('intelDrawdown', usd(dd.current_strategy_pnl)); setClass('intelDrawdown','intel-val '+cls(dd.current_strategy_pnl)); set('intelDrawdownNote', `Manual ${usd(dd.manual_unrealized_pnl)} · Combined equity drift ${usd(dd.combined_equity_drift)}. ${dd.note||''}`);
...
function drawPnlBarChart(canvas, rows, opts){
 ...
 const gap=Math.min(8, Math.max(2, iw/(data.length*8))); const bw=Math.max(4, Math.min(36, (iw - gap*(data.length-1))/data.length));
 const bars=[];
 data.forEach((d,i)=>{ const x=pad.l + i*(bw+gap); const yy=y(d.y); const bh=Math.abs(zero-yy); const top=Math.min(zero,yy); ctx.fillStyle=d.y>=0?'rgba(34,197,94,.88)':'rgba(239,68,68,.88)'; ctx.fillRect(x,top,bw,Math.max(2,bh)); bars.push({x,y:top,w:bw,h:Math.max(2,bh),r:d}); });
 // Overlay cumulative Larry realized P&L as a bright line with dots.
 const cx=(i)=>pad.l + i*(bw+gap) + bw/2;
...
function renderTradeMap(d){ const range=__tradeMapRange||'1M'; const priceCanvas=$('tradeMapPriceCanvas'), pnlCanvas=$('tradeMapPnlCanvas'); if(!priceCanvas||!pnlCanvas) return; const pricesRaw=selectTradeMapPrices(d,range); const trades=selectTradeMapTrades(d,range); const pricePts=pricesRaw.map(p=>({t:p.t,y:p.price,label:p.label})); const scale=drawLineChart(priceCanvas,pricePts,{range,valueFormat:'usd',valuePrefix:'$'}); const ctx=priceCanvas.getContext('2d'); __tradeMapMarkers=[]; if(scale && trades.length){ trades.forEach(r=>{ const k=tradeKind(r); const px = r.fill || (pricePts.length?pricePts.reduce((best,p)=>Math.abs(p.t-r.t)<Math.abs(best.t-r.t)?p:best,pricePts[0]).y:0); if(!px) return; const x=scale.x(Math.max(scale.xmin,Math.min(scale.xmax,r.t))); const y=scale.y(Math.max(scale.ymin,Math.min(scale.ymax,px))); let color='rgba(148,163,184,.95)'; if(k.kind==='buy') color='rgba(34,197,94,.98)'; else if(k.kind==='tp') color='rgba(245,158,11,.98)'; else if(k.kind==='stop') color='rgba(239,68,68,.98)'; ctx.fillStyle=color; ctx.strokeStyle='rgba(2,6,23,.95)'; ctx.lineWidth=2; ctx.beginPath(); if(k.kind==='buy'){ ctx.moveTo(x,y-8); ctx.lineTo(x-7,y+7); ctx.lineTo(x+7,y+7); } else if(k.kind==='stop'){ ctx.moveTo(x,y+8); ctx.lineTo(x-7,y-7); ctx.lineTo(x+7,y-7); } else { ctx.arc(x,y,7,0,Math.PI*2); } ctx.closePath(); ctx.fill(); ctx.stroke(); __tradeMapMarkers.push({x,y,r,k,px}); }); }
 ...
 drawPnlBarChart(pnlCanvas,realizedRows,{range,valueFormat:'usd',valuePrefix:'$'});
 const visibleNet=realizedRows.reduce((a,r)=>a+Number(r.net_realized_pnl_usd||0),0); const btcMove=(pricePts.length>=2)?((pricePts[pricePts.length-1].y/pricePts[0].y-1)*100):null; set('tmTradeCount', String(trades.length)); set('tmVisiblePnl', usd(visibleNet)); setClass('tmVisiblePnl','val '+cls(visibleNet)); set('tmBtcMove', btcMove==null?'—':pct(btcMove)); setClass('tmBtcMove','val '+cls(btcMove)); set('tmRangeLabel', range); set('tradeMapNote', `${range} view · ${pricePts.length} BTC price points · ${trades.length} Larry trade markers. Hover/tap markers for fill, reason, contracts, and P&L. Lower chart shows green/red per-trade realized net P&L bars plus a blue cumulative P&L line.`);
...
async function saveStrategyControls(){
 ...
 const payload={
   ...
   MAX_ABS_NET_CONTRACTS: maxConviction,
...
async function refresh(){try{const r=await fetch('/api/data?ts='+Date.now(),{cache:'no-store'}); const d=await r.json(); window.__LAST_DASH_DATA=d; if(!d.ok){$('errorBox').style.display='block';$('errorBox').textContent=d.error+'\n'+(d.trace||'');return} $('errorBox').style.display='none'; warn(d.warnings||[]); const c=d.capital||{}, pnl=d.pnl_summary||{}, sb=d.spot_balance||{}, fb=d.futures_balance||{}, cfg=d.config||{}, hb=d.heartbeat||{}, pm=d.perp_product||{}, mac=d.macro||{}, fills=d.fills||{}, perpMon=d.perp_signal_monitor||{};
 set('btcPrice',d.price?'$'+num(d.price,2):'—'); set('serverTime',d.server_time||'—'); set('productLine',`${cfg.SPOT_PRODUCT_ID||'BTC-USDC'} · ${cfg.PERP_PRODUCT_ID||'BIP-20DEC30-CDE'}`);
 ...
 set('startingCapital',usd(c.starting_combined_capital)); set('baselineSource',(c.baseline_set?('Source: '+(c.baseline_source||'set')+(c.started_at?' · '+c.started_at:'')):'Not set — use /api/set_capital_baseline')); set('currentCapital',usd(c.current_combined_capital)); set('currentCapitalMini',`Base fut ${usd(c.futures_base_equity)} + live UPL ${usd(c.futures_unrealized_pnl_adjustment)} + spot ${usd(c.spot_value)}`); const cleanTopPnl = pnl.net_book_impact || 0; const cleanTopReturn = (c.baseline_set && c.starting_combined_capital) ? (cleanTopPnl / c.starting_combined_capital * 100) : null; set('netPnl',c.baseline_set?usd(cleanTopPnl):'Baseline not set'); setClass('netPnl','val '+(c.baseline_set?cls(cleanTopPnl):'warn')); set('netReturn',(c.baseline_set && cleanTopReturn!=null)?pct(cleanTopReturn):'Baseline not set'); setClass('netReturn','val '+(c.baseline_set?cls(cleanTopReturn):'warn'));
 ...
 renderSpotToggleStatus(cfg); initTradeMapControls(); renderPerformanceAnalytics(d); renderTradeMap(d); renderLarryAccounting(d); renderSignals(d.signals); renderBTCTriggerMap(d); renderV12Transparency(d); renderRiskIntelligence(d); renderSignalLifecycle(d); renderEntryDiagnostics(d); renderKillSwitch(d.halt_state || (d.engine_state&&d.engine_state.kill_switch) || {}); renderPerpMonitor(perpMon,d); renderPositionRisk(d.position_risk); renderIAF(d.iaf_engine); renderPortfolio(d.portfolio_overlay); renderReconciler(d.position_reconciler); renderHistory(d.signal_history); renderMacro(mac,d.risk_gate); set('atrStop',(cfg.ATR_STOP_MULTIPLIER||1.5)+'x ATR'); set('tslAct',pct((cfg.TSL_ACTIVATION_PCT||0)*100)); set('tslTrail',pct((cfg.TSL_TRAIL_PCT||0)*100)); set('configSource',cfg.CONFIG_SOURCE||'—'); setControlValues(cfg);
