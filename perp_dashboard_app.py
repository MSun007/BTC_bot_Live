...
function renderEquityCurve(d){
 ...
 if(!base || rows.length<1){
   drawLineChart(canvas,[],{range:'ALL',valueFormat:'usd',valuePrefix:'$'});
 ...
 // Draw the curve.
 const scale=drawLineChart(canvas, series.map(p=>({t:p.t,y:p.y})), {range:'ALL',valueFormat:'usd',valuePrefix:'$'});
