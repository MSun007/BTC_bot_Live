...
def larry_trade_ledger_summary(engine_state: Dict[str, Any], tracking_start: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Fee-complete strategy P&L. Entry/add commissions are successful Larry
    # order costs even though those rows do not realize price P&L.
    ...
    running_net = 0.0
    for r in rows:
        r["strategy_net_impact_usd"] = None
        if r.get("ok"):
            impact = safe_float(r.get("gross_realized_pnl_usd"), 0.0) - safe_float(r.get("fees_usd"), 0.0)
            r["strategy_net_impact_usd"] = impact
            running_net += impact
        r["running_net_pnl_usd"] = running_net

    total_trades = len(realized_rows)
    exit_fees = sum(safe_float(r.get("fees_usd"), 0.0) for r in realized_rows)
    entry_add_fees = max(0.0, fees - exit_fees)
    allocated_entry_fee = (entry_add_fees / total_trades) if total_trades else 0.0
    completed_trade_nets = [
        safe_float(r.get("gross_realized_pnl_usd"), 0.0)
        - safe_float(r.get("fees_usd"), 0.0)
        - allocated_entry_fee
        for r in realized_rows
    ]
    wins = [v for v in completed_trade_nets if v > 0]
    losses = [v for v in completed_trade_nets if v < 0]
    gross_wins = sum(wins)
    ...
    return {
        ...
        "trade_stats": {
            ...
            "fee_allocation_method": "Historical entry/add fees allocated evenly across realized exits; total reconciles exactly to gross realized minus all Larry fees",
        },
        "execution_quality": {
...
.leg-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:10px}.leg-card{background:#0b0e14;border:1px solid rgba(255,255,255,.1);border-radius:15px;padding:12px}.leg-card.core{border-color:rgba(56,189,248,.45)}.leg-card.add{border-color:rgba(167,139,250,.45)}.leg-head{display:flex;justify-content:space-between;gap:8px;align-items:center}.leg-name{font-weight:1000}.leg-badge{font-size:.65rem;font-weight:950;padding:4px 7px;border-radius:999px;background:rgba(255,255,255,.07)}.leg-kv{display:grid;grid-template-columns:1fr auto;gap:6px;margin-top:8px;font-size:.72rem}.leg-kv span:nth-child(odd){color:var(--sub)}.leg-kv span:nth-child(even){font-weight:850}.version-note{font-size:.68rem;color:var(--muted);margin-top:8px}@media(max-width:760px){.leg-grid{grid-template-columns:1fr}}
...
<section class="card span12"><h2>Live Perp Position Risk Card <span class="method-badge">live Coinbase position</span></h2><div id="perpRiskCard"><div class="muted">Loading position risk…</div></div></section>
...
function renderSignalLifecycle(d){
 ...
 const liveContracts=Math.abs(Number(((d.position_risk||{}).contracts)||0));
 const ladder=Array.isArray(cfg.POSITION_SIZE_LADDER)?cfg.POSITION_SIZE_LADDER.map(Number).filter(x=>x>0).sort((a,b)=>a-b):[4];
 const executableTarget=liveContracts?(ladder.find(x=>x>liveContracts && x<=Number(lockTarget||ladder[ladder.length-1]))||liveContracts):(ladder[0]||4);
 ...
 set('lifeLockedNote', `Signal target ${lockTarget ?? '—'} · executable next ${executableTarget} contracts · armed ${fmtET(armed)} · confirmation ${(ph.confirmation_mode||'next_closed_candle')} · extension achieved ${ph.extension_achieved?'YES':'NO'}.`);
 ...
 set('lifeCommitNote', 'Entries confirm on the next closed candle. Phantom extension is monitored for sizing/add-ons, not required for entry.');
 const sig=d.signals||{};
...
function renderV12Transparency(d){
 ...
 set('v12CooldownNote', `Bridge ${bridgeCd}. LONG and SHORT cooldowns are tracked separately so one side does not suppress the other.`);
...
function tradeKind(r){ const reason=String(r.reason||'').toUpperCase(); const action=String(r.action||'').toUpperCase(); const net=Number(r.strategy_net_impact_usd ?? r.net_realized_pnl_usd ?? 0); if(action==='BUY' && r.gross_realized_pnl_usd==null) return {kind:'buy',label:reason.includes('ADD')?'ADD':'BUY',emoji:'🟢'}; if(reason.includes('TP')) return {kind:'tp',label:'TP',emoji:'🟡'}; if(reason.includes('STOP')||reason.includes('ATR')||reason.includes('TSL')||reason.includes('FLATTEN')) return {kind:'stop',label:reason.includes('ATR')?'ATR':reason.includes('TSL')?'TSL':'EXIT',emoji:'🔴'}; if(action==='SELL' && r.gross_realized_pnl_usd==null) return {kind:'buy',label:reason.includes('ADD')?'ADD':'SELL',emoji:'🟢'}; return {kind:net>=0?'tp':'stop',label:action||'TRADE',emoji:net>=0?'🟡':'🔴'}; }
...
function drawPnlBarChart(canvas, rows, opts){
 ...
 const data=(rows||[]).filter(r=>r.ok && r.strategy_net_impact_usd!==null && r.strategy_net_impact_usd!==undefined).map((r,i)=>({
   ...r, idx:i+1, y:Number(r.strategy_net_impact_usd||0), label:(r.timestamp_et||fmtET(r.timestamp)||('T'+(i+1)))
 }));
 ...
 const wins=data.filter(d=>d.y>0).length, losses=data.filter(d=>d.y<0).length, net=data[data.length-1].cum; ctx.font='12px system-ui'; ctx.fillStyle='rgba(226,232,240,.84)'; ctx.fillText(`Bars: per-order net impact · Line: cumulative fee-complete P&L · ${usd(net)}`, pad.l, 15);
...
function renderLarryAccounting(d){
 ...
 if(body){
   ...
   body.innerHTML=rows.length?rows.map(r=>{
     const net = r.strategy_net_impact_usd;
...
async function refresh(){try{const r=await fetch('/api/data?ts='+Date.now(),{cache:'no-store'}); const d=await r.json(); window.__LAST_DASH_DATA=d; if(!d.ok){$('errorBox').style.display='block';$('errorBox').textContent=d.error+'\n'+(d.trace||'');return} $('errorBox').style.display='none'; warn(d.warnings||[]); const c=d.capital||{}, pnl=d.pnl_summary||{}, sb=d.spot_balance||{}, fb=d.futures_balance||{}, cfg=d.config||{}, hb=d.heartbeat||{}, pm=d.perp_product||{}, mac=d.macro||{}, fills=d.fills||{}, perpMon=d.perp_signal_monitor||{};
 ...
 const subsystemHealth=es.api_health||{};
 for(const [id,label] of [['coinbaseHealthPill','COINBASE'],['storageHealthPill','STORAGE']]){if(!$(id)){const p=document.createElement('span');p.id=id;p.className='pill blue';$('botHealth')?.after(p)}}
 const cbHealth=String(subsystemHealth.coinbase_status||'UNKNOWN').toUpperCase(), gcsHealth=String(subsystemHealth.gcs_status||'UNKNOWN').toUpperCase();
 pill('coinbaseHealthPill',`COINBASE ${cbHealth}${subsystemHealth.consecutive_failures?' · '+subsystemHealth.consecutive_failures:''}`,cbHealth==='HEALTHY'?'good':cbHealth==='DEGRADED'?'warn':'bad');
 pill('storageHealthPill',`STORAGE ${gcsHealth}${subsystemHealth.gcs_consecutive_failures?' · '+subsystemHealth.gcs_consecutive_failures:''}`,gcsHealth==='HEALTHY'?'good':gcsHealth==='DEGRADED'?'warn':'bad');
 ...
 renderSpotToggleStatus(cfg); initTradeMapControls(); renderPerformanceAnalytics(d); renderTradeMap(d); renderLarryAccounting(d); renderSignals(d.signals); renderBTCTriggerMap(d); renderV12Transparency(d); renderRiskIntelligence(d); renderSignalLifecycle(d); renderLarryMindset(d); renderEntryDiagnostics(d); renderKillSwitch(d.halt_state || (d.engine_state&&d.engine_state.kill_switch) || {}); renderPerpMonitor(perpMon,d); renderPositionRisk(d.position_risk); renderIAF(d.iaf_engine); renderPortfolio(d.portfolio_overlay); renderReconciler(d.position_reconciler); renderPositionLegs(d); renderHistory(d.signal_history); renderMacro(mac,d.risk_gate); renderEquityCurve(d);
