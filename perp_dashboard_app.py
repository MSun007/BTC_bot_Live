...
async function refresh(){try{const r=await fetch('/api/data?ts='+Date.now(),{cache:'no-store'}); const d=await r.json(); window.__LAST_DASH_DATA=d; if(!d.ok){$('errorBox').style.display='block';$('errorBox').textContent=d.error+'\n'+(d.trace||'');return} $('errorBox').style.display='none'; warn(d.warnings||[]); const c=d.capital||{}, pnl=d.pnl_summary||{}, sb=d.spot_balance||{}, fb=d.futures_balance||{}, cfg=d.config||{}, hb=d.heartbeat||{}, pm=d.perp_product||{}, mac=d.macro||{}, fills=d.fills||{}, perpMon=d.perp_signal_monitor||{};
 ...
 set('serverTime',(d.server_time||'—')+_fresh); set('productLine',`${cfg.SPOT_PRODUCT_ID||'BTC-USDC'} · ${cfg.PERP_PRODUCT_ID||'BIP-20DEC30-CDE'}`);
 ...
 set('startingCapital',usd(c.starting_combined_capital)); set('baselineSource',(c.baseline_set?('Source: '+(c.baseline_source||'set')+(c.started_at?' · '+c.started_at:'')):'Not set — use /api/set_capital_baseline')); set('currentCapital',usd((c.starting_combined_capital||0)+(pnl.net_book_impact||0))); set('currentCapitalMini',`Baseline ${usd(c.starting_combined_capital)} + Larry P&L ${usd(pnl.net_book_impact)}`); const cleanTopPnl = pnl.net_book_impact || 0; const cleanTopReturn = (c.baseline_set && c.starting_combined_capital) ? (cleanTopPnl / c.starting_combined_capital * 100) : null; set('netPnl',c.baseline_set?usd(cleanTopPnl):'Baseline not set'); setClass('netPnl','val '+(c.baseline_set?cls(cleanTopPnl):'warn')); set('netReturn',(c.baseline_set && cleanTopReturn!=null)?pct(cleanTopReturn):'Baseline not set'); setClass('netReturn','val '+(c.baseline_set?cls(cleanTopReturn):'warn'));
